  ...
  res.json({
    ...
    info: {
      ...
      'x-pricing': {
        ...
        '/batch-score': 0.005,
        '/v1/execution-gate': 0.004,
        '/register-webhook': 0.002,
      },
    },
    servers: [{ url: 'https://orbis-apis.onrender.com/phone-validation', description: 'Production' }],
    paths: {
      '/v1/validate': { post: { summary: 'Validate a phone number', tags: ['Validation'], 'x-agent-callable': true, requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { phone: { type: 'string' }, country_code: { type: 'string' } }, required: ['phone'] } } } }, responses: { 200: { description: 'valid, E.164, country, line_type, risk' } } } },
