...
openapiRouter.get('/', (_req: Request, res: Response) => {
  res.json({
    ...
    paths: {
      ...
      '/batch': { post: { operationId: 'scoreLeadsBatch', summary: 'Batch score multiple leads for pipeline prioritization', requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['leads'], properties: { leads: { type: 'array', items: { type: 'object' }, maxItems: 50 }, icp: { type: 'object' } } } } } }, responses: { '200': { description: 'Batch scores', content: { 'application/json': { schema: { type: 'object', properties: { results: { type: 'array', items: { type: 'object' } }, average_score: { type: 'number' }, grade_distribution: { type: 'object' }, chain_to: { type: 'array', items: { type: 'string' } }, recommended_actions_priority_order: { type: 'array', items: { type: 'string' } }, confidence_per_section: { type: 'object', additionalProperties: { type: 'number' } }, privacy } } } } }, '400': { description: 'Missing leads' }, '500': { description: 'Batch failed' } } } }
    }
  });
});
