...
export async function analyzeImage(req: AnalyzeRequest & { session_id?: string }): Promise<AnalyzeResponse> {
  ...
  return {
    ...
    confidence_per_section: {
      ...
      semantic: 0.83,
      extraction_completeness: parsed.caption && parsed.tags ? 0.91 : 0.72,
    },
    visual_reasoning: {
      document_consistency: 0.85,
    ...
    retryable: false,
    orchestration_hints: {
      next_step: parsed.ocr ? 'entity-link' : 'analyze-complete',
      suggested_chain: ['entity-link', 'compare'],
