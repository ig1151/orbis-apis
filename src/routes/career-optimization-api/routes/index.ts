...
router.get('/discovery', (_req, res) => {
  res.json({
    ...
    pricing: { model: 'per_call', unit_cost_usd: 0.005, tier: 'utility', tier_label: 'Agent Utility', description: 'Resume scoring, optimization, and career strategy', },
  ...
  try {
    ...
    res.json({
      ...buildRuntime(req, {
        ...
        orchestration_hints: { can_chain: true, suggested_next: ['/career-optimization/score-resume', '/proposal-generation/generate', '/outreach-execution/generate'], requires_review: false },
      }),
      success: true,
            score: ai.score ?? null,
