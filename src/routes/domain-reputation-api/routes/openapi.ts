...
router.get('/', (_req: Request, res: Response) => {
  res.json({
    openapi: '3.1.0',
    info: { title: 'Domain Reputation API', version: '2.0.0', description: 'Score domain reputation across threat intelligence sources.', 'x-agent-callable': true, 'x-mcp-compatible': true },
    servers: [{ url: 'https://orbis-apis.onrender.com/domain-reputation', description: 'Production' }],
    security: [{ ApiKeyAuth: [] }],
    paths: {
      '/score': { post: { operationId: 'score_reputation', summary: 'Score domain reputation', tags: ['Intelligence'], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['input'], properties: { input: { type: 'string', example: 'example.com' }, options: { type: 'object' } } } } } }, responses: { '200': { description: 'Reputation score', content: { 'application/json': { schema: { $ref: '#/components/schemas/StandardResponse' } } } }, '400': { $ref: '#/components/responses/BadRequest' }, '500': { $ref: '#/components/responses/ServerError' } } } },
      '/check': { post: { operationId: 'check_domain', summary: 'Check domain against blacklists and WHOIS', tags: ['Intelligence'], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['input'], properties: { input: { type: 'string' }, options: { type: 'object' } } } } } }, responses: { '200': { description: 'Check result', content: { 'application/json': { schema: { $ref: '#/components/schemas/StandardResponse' } } } }, '400': { $ref: '#/components/responses/BadRequest' }, '500': { $ref: '#/components/responses/ServerError' } } } },
