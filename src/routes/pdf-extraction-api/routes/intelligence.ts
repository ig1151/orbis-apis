...
router.post('/analyze', async (req: Request, res: Response) => {
  ...
  try {
    const data = await callClaude(`You are an expert pdf-extraction analysis engine. Analyze the input and return ONLY a valid JSON object with relevant structured fields.
...
router.post('/execution-gate', async (req: Request, res: Response) => {
  ...
  const start = Date.now();
  try {
    const data = await callClaude(`You are an autonomous agent execution gate for pdf-extraction. Determine whether the agent should proceed and return ONLY a valid JSON object with these keys:
- execute: boolean
