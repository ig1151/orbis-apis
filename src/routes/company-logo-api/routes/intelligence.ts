...
router.get('/', (_req, res) => {
  res.json({
    name: ' API',
    info: '/company-logo/info',
    openapi: '/company-logo/openapi.json',
    health: 'ok'
  });
});
...
router.post('/analyze', async (req: Request, res: Response) => {
  ...
  try {
    const data = await callClaude(`You are an expert company-logo analysis engine. Analyze the input and return ONLY a valid JSON object with relevant structured fields.
...
// ── POST /analyze-company-logo (one-call workflow) ─────────────────────────────────
router.post('/analyze-company-logo', async (req: Request, res: Response) => {
  const { input, context } = req.body;
  if (!input) { res.status(400).json({ error: 'Provide input' }); return; }
  const start = Date.now();
  try {
    const data = await callClaude(`You are a complete company-logo intelligence engine. Perform a full analysis and return ONLY a valid JSON object with ALL relevant fields including:
- summary: string
...
Return only the JSON object:`, 2000) as Record<string, unknown>;
    ...
    const message = err instanceof Error ? err.message : 'Failed';
    logger.error({ endpoint: 'analyze-company-logo', err }, message);
    res.status(500).json({ error: message });
  }
});
...
router.post('/execution-gate', async (req: Request, res: Response) => {
  ...
  const start = Date.now();
  try {
    const data = await callClaude(`You are an autonomous agent execution gate for company-logo. Determine whether the agent should proceed and return ONLY a valid JSON object with these keys:
- execute: boolean
- confidence: number (0-1)
- risk_level: string (high|medium|low)
- blocking_flags: array of strings
