...
router.get('/', (_req: Request, res: Response) => {
  res.json({ name: 'Broken Link Checker API', info: '/broken-link-checker/info', openapi: '/broken-link-checker/openapi.json', health: 'ok' });
});
...
router.post('/batch', async (req: Request, res: Response) => {
  ...
  try {
    const raw = await callClaude(`You are an expert Broken Link Checker API engine performing batch link checking.\nInputs: ${JSON.stringify(inputs)}\nOptions: ${JSON.stringify(options || {})}\nReturn ONLY valid JSON with keys: success, request_id, data (results array each with url/status_code/is_broken/error, total_checked, broken_count, broken_urls array), confidence (score 0-1, reason, per_section), provenance (provider, retrieved_at ISO8601, source_type enum ai_generated|cached|live_scan|api_call), cache (recommended_ttl_seconds, retryable, cache_recommended), recommended_next_api (array: api, endpoint, reason), recommended_actions_priority_order (array: priority high|medium|low, action, reason), execution_metadata (latency_ms, model, automation_safe). No markdown.`);
    res.json(parseJSON(raw));
  } catch (e: any) { res.status(500).json({ error: e.message, code: 'UPSTREAM_ERROR', retryable: true }); }
});
...
router.post('/execution-gate', (_req: Request, res: Response) => {
  ...
  if (!input) return res.status(400).json({ error: 'input is required', code: 'MISSING_INPUT', retryable: false });
  res.json({ success: true, request_id: rid(), execution_ready: true, input, objective: objective || 'check', next_api: 'broken-link-checker', next_endpoint: '/link-intelligence', blocking_flags: [], confidence: { score: 0.98, reason: 'Input valid', per_section: { execution_ready: 0.98 } }, provenance: { provider: 'system', retrieved_at: new Date().toISOString(), source_type: 'api_call' }, recommended_next_api: [{ api: 'broken-link-checker', endpoint: '/link-intelligence', reason: 'Full intelligence in one call' }], recommended_actions_priority_order: [{ priority: 'high', action: 'Call /link-intelligence', reason: 'Single-request analysis' }], execution_metadata: { latency_ms: 1, model: 'system', automation_safe: true } });
});
