    ...
    paths: {
      ...
      '/top': { get: { operationId: 'getTopYields', summary: 'Get top stablecoin yield opportunities with AI recommendation', 'x-agent-callable': true,
        parameters: [
          ...
          { name: 'chain', in: 'query', required: false, schema: { type: 'string', description: 'Filter by chain e.g. ethereum, base' } },
        ...
        responses: { '200': { description: 'Top yield pools with AI recommendation', content: { 'application/json': { schema: { type: 'object', properties: {
