...
router.post('/estimate', async (req: Request, res: Response) => {
  ...
  try {
    const raw = await callClaude(`You are an expert Rate Limit Estimator API engine performing: estimate.\nInput: "${input}"\nOptions: ${JSON.stringify(options || {})}\nReturn ONLY valid JSON: success, request_id, data (estimated_rpm number, estimated_rph number, estimated_rpd number, rate_limit_headers_detected array, x_ratelimit_limit, x_ratelimit_remaining, x_ratelimit_reset, retry_after_seconds, tier_detected, concurrency_limit, burst_allowance, recommended_safe_rpm number, throttle_strategy enum fixed_window|sliding_window|token_bucket|leaky_bucket|unknown), confidence (score 0-1, reason, per_section), provenance (provider, retrieved_at ISO8601, source_type enum ai_generated|cached|live_scan|api_call), cache (recommended_ttl_seconds, retryable, cache_recommended), recommended_next_api (array: api, endpoint, reason), recommended_actions_priority_order (array: priority high|medium|low, action, reason), execution_metadata (latency_ms, model, automation_safe). No markdown.`);
...
router.post('/analyze', async (req: Request, res: Response) => {
  ...
  if (!input) return res.status(400).json({ error: 'input is required', code: 'MISSING_INPUT', retryable: false });
  try {
    const raw = await callClaude(`You are an expert Rate Limit Estimator API engine performing: analyze.\nInput: "${input}"\nOptions: ${JSON.stringify(options || {})}\nReturn ONLY valid JSON: success, request_id, data (rate_limit_violations_detected array, violations_per_hour, back_pressure_events_count, adaptive_throttle_recommended boolean, recommended_concurrency number, queue_depth_estimate number, latency_p50_ms, latency_p99_ms, efficiency_score 0-100, optimization_suggestions array), confidence (score 0-1, reason, per_section), provenance (provider, retrieved_at ISO8601, source_type enum ai_generated|cached|live_scan|api_call), cache (recommended_ttl_seconds, retryable, cache_recommended), recommended_next_api (array: api, endpoint, reason), recommended_actions_priority_order (array: priority high|medium|low, action, reason), execution_metadata (latency_ms, model, automation_safe). No markdown.`);
    res.json(parseJSON(raw));
  } catch (e: any) { res.status(500).json({ error: e.message, code: 'UPSTREAM_ERROR', retryable: true }); }
});
