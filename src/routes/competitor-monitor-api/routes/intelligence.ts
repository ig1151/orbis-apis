...
router.post('/analyze-competitive-landscape', async (req: Request, res: Response) => {
  ...
  try {
    const result = await callClaude(
      ...
      `Run a full competitive landscape analysis and return JSON with fields: market_overview (size, growth_rate, maturity), competitive_intensity (score 0-100, rating low/medium/high/extreme), your_position (rank, strengths array, vulnerabilities array, differentiation_score), competitor_profiles (array with name, threat_level, market_share, key_strength, key_weakness), threat_matrix (top 3 threats with severity and response), opportunity_map (array with opportunity, barrier, potential_impact), strategic_recommendations (array with priority, action, timeline, expected_outcome), early_warning_signals (array), executive_summary, confidence_score (0-100). Company: ${your_company} Competitors: ${JSON.stringify(competitors || [])} Industry: ${industry} Goals: ${JSON.stringify(strategic_goals || [])}`
    );
