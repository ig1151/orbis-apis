...
router.get('/', (_req: Request, res: Response) => {
  res.json({
    ...
    endpoints: [
      { method: 'POST', path: '/generate-key', summary: 'Generate a random 256-bit key', price_usdc: 0.003 },
      { method: 'POST', path: '/encrypt', summary: 'Encrypt plaintext with a provided key', price_usdc: 0.01 },
      { method: 'POST', path: '/decrypt', summary: 'Decrypt ciphertext with key + iv + auth tag', price_usdc: 0.01 },
      { method: 'POST', path: '/lookup', summary: 'ONE-CALL: generate key + encrypt + return full reusable bundle', price_usdc: 0.02 },
    ],
    pricing: [
      { path: '/generate-key', price_usdc: 0.003, currency: 'USDC' },
      { path: '/encrypt', price_usdc: 0.01, currency: 'USDC' },
      { path: '/decrypt', price_usdc: 0.01, currency: 'USDC' },
