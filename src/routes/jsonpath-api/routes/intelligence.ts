...
const CHAIN_TO = [
  { api: 'json-pointer', reason: 'Resolve a single matched path as an exact RFC 6901 pointer.' },
  { api: 'json-patch', reason: 'Apply an RFC 6902 patch at a located path.' },
];
const INVALIDATORS = [
...
router.post('/query', (req: Request, res: Response) => {
  ...
  const v = r.result;
  respond(res, t0, { ...v, ...TAIL({ query: 1 }, [`${v.match_count} match(es) for ${v.path}.`]) });
});
...
router.post('/value', (req: Request, res: Response) => {
  ...
  respond(res, t0, { path: v.path, found, value: found ? v.matches[0].value : null, matched_path: found ? v.matches[0].path : null, match_count: v.match_count, ...TAIL({ query: 1 }, [found ? `First match at ${v.matches[0].path}.` : `No match for ${v.path}.`]) });
});
...
router.post('/lookup', (req: Request, res: Response) => {
  ...
  respond(res, t0, {
    ...
    ...TAIL({ query: 1 }, [`${v.match_count} match(es) for ${v.path}.`]),
  });
});
