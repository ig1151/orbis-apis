...
router.get('/discovery', (_req, res) => {
  res.json({
    ...
    pricing: { model: 'per_call', unit_cost_usd: 0.02, tier: 'infrastructure', tier_label: 'Autonomous Infrastructure', description: 'Cryptographic agent identity and trust scoring', },
...
router.post('/revoke', (req: Request, res: Response) => {
  ...
  res.set("x-trace-id", buildRuntime(req).trace_id);
  res.set("x-execution-id", buildRuntime(req).execution_id);
  res.json({ ...buildRuntime(req), success: true, revoked: 'revoked_value', agent_id: 'agent_id_value', revoked_at: 'revoked_at_value', computed_at: new Date().toISOString() });
});
