...
router.get('/', (_req: Request, res: Response) => {
  res.json({
    ...
    endpoints: [
      { method: 'POST', path: '/scan', summary: 'Cross-chain price scan for a token across all supported chains', price_usdc: 0.005 },
      { method: 'POST', path: '/routes', summary: 'All profitable cross-chain routes ranked by net profit', price_usdc: 0.006 },
      { method: 'POST', path: '/lookup', summary: 'ONE-CALL: full cross-chain arb with bridge recommendation and execution steps', price_usdc: 0.015 },
    ],
    pricing: { free_tier: { requests_per_day: 100, requests_per_month: 3000 }, pay_per_call: { scan: '$0.005', routes: '$0.006', lookup: '$0.015' } },
