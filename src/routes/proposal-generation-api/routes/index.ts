...
router.get('/discovery', (_req, res) => {
  res.json({
    ...
    pricing: { model: 'per_call', unit_cost_usd: 0.008, tier: 'utility', tier_label: 'Agent Utility', description: 'Business proposal, RFP response, and SOW generation', },
...
router.post('/generate', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const _resp = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: 'anthropic/claude-sonnet-4-5', max_tokens: 1500, response_format: { type: 'json_object' },
        messages: [
    ...
    res.json({
      ...
      success: true,
            proposal: ai.proposal ?? ai.content ?? ai.proposal_text ?? ai.full_proposal ?? null,
      sections: ai.sections ?? ai.section_names ?? ai.table_of_contents ?? null,
      word_count: ai.word_count ?? ai.wordCount ?? ai.total_words ?? null,
      executive_summary: ai.executive_summary ?? ai.executiveSummary ?? ai.summary ?? ai.overview ?? null,
...
router.post('/workflow/start', (req: Request, res: Response) => {
  ...
  const defaultSteps = ["validate_inputs", "generate_content", "score_quality", "apply_tone", "finalize"];
