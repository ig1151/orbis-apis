...
const ApprovalRow = {
  type: 'object', additionalProperties: false,
  required: ['token', 'spender', 'is_unlimited', 'risk_score', 'risk_band', 'revoke_recommended', 'reasons'],
  properties: {
    ...
    revoke_recommended: { type: 'boolean' }, reasons: strArr,
  },
};

const ScanCore = {
  type: 'object',
  required: ['total_approvals', 'unlimited_count', 'flagged_spender_count', 'stale_count', 'unverified_spender_count', 'exposure_score', 'exposure_band', 'approvals', 'revoke_priority', 'risk_disclaimer'],
  properties: {
...
const ROWS = [
  { token: 'USDC', spender: 'UnknownRouter', is_unlimited: true, risk_score: 60, risk_band: 'high', revoke_recommended: true, reasons: ['Unlimited allowance — the spender can move your entire balance of this token.', 'Spender contract is unverified — source code cannot be reviewed.', 'Stale approval — unused for 400d; likely safe to revoke.'] },
  { token: 'WETH', spender: 'Uniswap V3 Router', is_unlimited: false, risk_score: 0, risk_band: 'low', revoke_recommended: false, reasons: ['Bounded, recently-used approval to a known spender — low concern.'] },
];
const CORE = {
  total_approvals: 2, unlimited_count: 1, flagged_spender_count: 0, stale_count: 1, unverified_spender_count: 1,
  exposure_score: 60, exposure_band: 'high', approvals: ROWS, revoke_priority: [ROWS[0]],
...
const ACTS = [
  'Scanned 2 approval(s): exposure 60/100 (high); 1 recommended for revocation.',
