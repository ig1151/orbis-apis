...
router.get('/', (_req: Request, res: Response) => {
  res.json({
    ...
    info: {
      ...
      'x-pricing': { free_tier: { requests_per_day: 100, requests_per_month: 3000 }, pay_per_call: { track: '$0.003', scan: '$0.004', lookup: '$0.006' } },
    ...
    paths: {
      ...
      '/track': {
        post: {
          ...
          summary: 'Track a specific wallet — classification, recent activity, holdings, and entry/exit signals',
          requestBody: {
            required: true,
            content: {
              'application/json': {
                ...
                'application/json': {
                  schema: {
                    ...
                    properties: {
                      ...
                      holdings_snapshot: { type: 'array', items: { type: 'object', properties: { token: { type: 'string' }, amount: { type: 'number' }, value_usd: { type: 'number' }, pct_portfolio: { type: 'number' } } } },
                      behavior_patterns: { type: 'object', properties: { trading_style: { type: 'string' }, avg_hold_days: { type: 'number' }, win_rate_pct: { type: 'number' }, recent_bias: { type: 'string' } } },
                      entry_exit_signals: { type: 'object', properties: { current_signal: { type: 'string' }, signal_strength: { type: 'string' }, notable_observation: { type: 'string' } } },
                ...
                'application/json': {
                  schema: {
                    ...
                    properties: {
                      ...
                      movements: { type: 'array', items: movementItem },
                      market_summary: { type: 'object', properties: { net_whale_flow_usd: { type: 'number' }, dominant_action: { type: 'string' }, top_tokens_by_flow: { type: 'array', items: { type: 'string' } }, overall_signal: { type: 'string' } } },
                      financial_disclaimer: { type: 'string' },
                      paper_mode_recommended: { type: 'boolean' },
                      confidence_per_section: confidence,
                      recommended_actions_priority_order: actions,
      ...
      '/lookup': {
        post: {
          ...
          summary: 'ONE-CALL: top whale wallets + recent moves + aggregate entry/exit signal for a token',
          'x-one-call': true,
          requestBody: {
            required: true,
            content: {
                ...
                'application/json': {
                  schema: {
                    ...
                    properties: {
                      ...
                      token: { type: 'string' },
                      chain: { type: 'string' },
                      top_whale_wallets: { type: 'array', items: { type: 'object', properties: { address: { type: 'string' }, tier: { type: 'string' }, holdings_usd: { type: 'number' }, pct_supply: { type: 'number' }, recent_action: { type: 'string' }, action_strength: { type: 'string' } } } },
                      recent_movements: { type: 'array', items: movementItem },
                      concentration_risk: { type: 'object', properties: { top_10_pct_supply: { type: 'number' }, top_50_pct_supply: { type: 'number' }, risk_level: { type: 'string' }, risk_note: { type: 'string' } } },
                      aggregate_signal: { type: 'object', properties: { signal: { type: 'string', enum: ['strong_buy', 'buy', 'neutral', 'sell', 'strong_sell'] }, whale_consensus: { type: 'string' }, short_term_outlook: { type: 'string' }, key_insight: { type: 'string' } } },
                      financial_disclaimer: { type: 'string' },
                      paper_mode_recommended: { type: 'boolean' },
                      confidence_per_section: confidence,
                      recommended_actions_priority_order: actions,
