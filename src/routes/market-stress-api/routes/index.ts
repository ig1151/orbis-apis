...
router.get('/discovery', (_req, res) => {
  res.json({
    ...
    pricing: { model: 'per_call', unit_cost_usd: 0.01, tier: 'platform', tier_label: 'Platform Intelligence', description: 'Composite market stress index and scenario simulation', },
...
router.get('/history', (req: Request, res: Response) => {
  ...
  res.set("x-trace-id", buildRuntime(req).trace_id);
  res.set("x-execution-id", buildRuntime(req).execution_id);
  res.json({ ...buildRuntime(req), success: true, timeseries: 'timeseries_value', peak_stress: 'peak_stress_value', avg_stress: 'avg_stress_value', human_approval_required: true, computed_at: new Date().toISOString() });
});
