...
router.get('/discovery', (_req, res) => {
  res.json({
    ...
    pricing: { model: 'per_call', unit_cost_usd: 0.006, tier: 'utility', tier_label: 'Agent Utility', description: 'Personalized outreach generation and sequence management', },
  ...
  try {
    ...
    res.json({
      ...buildRuntime(req, {
        ...
        orchestration_hints: { can_chain: true, suggested_next: ['/outreach-execution/sequence', '/career-optimization/strategy', '/proposal-generation/generate'], requires_review: false },
      }),
      success: true,
