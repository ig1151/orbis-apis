...
router.get('/', (_req: Request, res: Response) => {
  res.json({
    ...
    paths: {
      ...
      '/sanctions-screen': { post: { operationId: 'sanctionsScreen', summary: 'Screen agent or wallet against OFAC, UN, EU sanctions lists with match confidence and blocking decision', requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['agent_id', 'wallet_address'], properties: { agent_id: { type: 'string' }, wallet_address: { type: 'string' }, name: { type: 'string' }, jurisdiction: { type: 'string' }, screen_lists: { type: 'array', items: { type: 'string' } } } } } } }, responses: { '200': { description: 'Sanctions screening result' }, '400': { description: 'Missing agent_id or wallet_address' }, '500': { description: 'Screening failed' } } } },
      '/verify-counterparty': { post: { operationId: 'verifyCounterparty', summary: 'Verify identity and trustworthiness of a payment counterparty with trust score and interaction limits', requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['agent_id', 'counterparty_id'], properties: { agent_id: { type: 'string' }, counterparty_id: { type: 'string' }, wallet_address: { type: 'string' }, verification_level: { type: 'string', enum: ['basic', 'standard', 'enhanced', 'kyc'] }, context: { type: 'object' } } } } } }, responses: { '200': { description: 'Counterparty verification result' }, '400': { description: 'Missing agent_id or counterparty_id' }, '500': { description: 'Verification failed' } } } },
      '/aml-check': { post: { operationId: 'amlCheck', summary: 'Anti-money laundering check with structuring detection, velocity flags, SAR/CTR requirements and jurisdiction risk', requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['agent_id', 'wallet_address', 'amount_usdc'], properties: { agent_id: { type: 'string' }, wallet_address: { type: 'string' }, amount_usdc: { type: 'number' }, transaction_pattern: { type: 'object' }, jurisdiction: { type: 'string' } } } } } }, responses: { '200': { description: 'AML check result' }, '400': { description: 'Missing required fields' }, '500': { description: 'AML check failed' } } } },
      '/transaction-risk': { post: { operationId: 'transactionRisk', summary: 'Standalone transaction risk assessment scoring counterparty, amount, pattern, jurisdiction and timing risk', requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['agent_id', 'from_wallet', 'to_wallet', 'amount_usdc'], properties: { agent_id: { type: 'string' }, from_wallet: { type: 'string' }, to_wallet: { type: 'string' }, amount_usdc: { type: 'number' }, purpose: { type: 'string' }, metadata: { type: 'object' } } } } } }, responses: { '200': { description: 'Transaction risk assessment' }, '400': { description: 'Missing required fields' }, '500': { description: 'Risk assessment failed' } } } },
    },
  });
});
