...
Return JSON: { "execute": true, "entity_id": "string", "action_type": "string", "compliance_score": 0.9, "risk_score": 0.1, "risk_level": "low|medium|high|critical", "checks_passed": {"kyc":true,"sanctions":true,"aml":true,"jurisdiction":true}, "blocking_flags": ["string"], "warnings": ["string"], "human_approval_required": false, "recommended_action": "proceed|require_approval|block|escalate", "audit_entry": {"timestamp":"string","hash":"string"}, "chain_to": ["string"], "confidence_per_section": {"compliance":0.95,"risk":0.9}, "recommended_actions_priority_order": ["string"], "privacy": {"data_stored":false,"retention":"none"} }`);
    res.json(parseJSON(raw));
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
