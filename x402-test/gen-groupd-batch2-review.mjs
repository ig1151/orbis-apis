// Assembles the Group D finance batch 2 ChatGPT review bundle:
...
// OpenAPI requestBody example; error cases mirror x402-test/smoke-groupd-batch2.mjs.
...
const ERROR_CALLS = {
  'returns-analytics': [
    ['POST /summary (error: invalid input)', '/summary', { values: 'nope' }],
    ['POST /cagr (error: invalid input)', '/cagr', { begin_value: 0 }],
  ],
  'max-drawdown': [
    ['POST /analyze (error: invalid input)', '/analyze', { values: 'nope' }],
  ],
  'value-at-risk': [
    ['POST /historical (error: invalid input)', '/historical', { returns: 'nope' }],
  ],
  'compound-interest': [
    ['POST /future-value (error: invalid input)', '/future-value', { principal: 'nope' }],
  ],
  'wacc': [
    ['POST /wacc (error: invalid input)', '/wacc', { equity_weight: 'nope' }],
    ['POST /capm (error: invalid input)', '/capm', { beta: 'nope' }],
  ],
};
...
p('# Group D Finance Batch 2 — ChatGPT Review Bundle (Orbis A+ finance/quant APIs)');
p('');
p(`Date: 2026-06-18 · shipped in PR #59 · tsc clean · smoke 45/45 green · live-verified on Render`);
p('');
p('Five deterministic, agent-native finance/quant APIs: **returns-analytics**, **max-drawdown**, **value-at-risk**, **compound-interest**, **wacc**. Built on the shared `_aplus` scaffold + finance tail (`specparts-finance.ts`, required `financial_disclaimer`); shared quant math in `_aplus/finance.ts`.');
p('');
p('## Please grade each API (A+/A/B/...) on:');
p('1. **Correctness** of the deterministic finance math (CAGR/total/annualized return from an equity curve; max drawdown peak/trough/recovery + Calmar; historical α-quantile VaR + expected shortfall and parametric Gaussian VaR; compound future value with periodic contributions + nominal→effective-annual-rate; WACC by market-value weights + CAPM cost of equity).');
p('2. **A+ envelope completeness** (trace_id/request_id, computed_at, success, latency_ms, confidence_score, confidence_per_section, recommended_actions_priority_order, chain_to, privacy, execution_metadata side_effects/compute_class, financial_disclaimer; reasoning + capabilities on /lookup).');
...
const outPath = `${ROOT}/groupd-finance-batch2-chatgpt-review.md`;
